...
export const createProduct = async (req, res, next) => {
  ...
  const subCat = await subCategoryModel.findOne({_id:subCategoryId, categoryId:categoryId})
  if(!subCat){
    return next(new Error('invalid subCategory or category', {cause:404}));
  }
  const brand = await brandModel.findOne({_id:brandId, categoryId:categoryId})
  if(!brand){
  ...
  const {secure_url, public_id} = await cloudinary.uploader.upload(mainImage[0].path, {folder:`${process.env.APP_NAME}/product/mainImage`});
  req.body.mainImage = {secure_url, public_id}
  if(subImages){
    req.body.subImages = [];
    for(const image of subImages){
      const {secure_url, public_id} = await cloudinary.uploader.upload(image.path, {folder:`${process.env.APP_NAME}/product/subImages`});
...
export const updateProduct = async (req, res, next) => {
  ...
  if (!product)
    return next(
      new Error(`invalid product id ${productId}`, { cause: 400 })
    );
  ...
  const subCat = await subCategoryModel.findOne({_id:subCategoryId, categoryId:req.body.categoryId});
  if(subCat){
    
    product.subCategoryId = subCat._id;
  }
  
  const brand = await brandModel.findOne({_id:brandId, categoryId:req.body.categoryId , subCategoryId:req.body.subCategoryId})
  if(brand){
  ...
  if (req.body.description) {
    if (product.description === req.body.description)
      return next(new Error(`old description matches the new description`, { cause: 409 }));
  ...
  if (req.body.discount) {
    if (product.discount === req.body.discount)
      return next(new Error(`old discount matches the new discount`, { cause: 409 }));
  ...
  if (req.body.colors) {
    if (product.colors === req.body.colors)
      return next(new Error(`old colors matches the new colors`, { cause: 409 }));
  ...
  product.finalPrice = product.price - product.price* (product.discount || 0 )/100.0;
...
export const getAllProducts = async (req, res, next) => {
  const categories = await productModel.find({isDeleted:false});
...
export const softDelete = async (req, res, next) => {
  ...
  const product = await productModel.findOneAndUpdate({_id: productId}, {isDeleted:true}, {new:true});
